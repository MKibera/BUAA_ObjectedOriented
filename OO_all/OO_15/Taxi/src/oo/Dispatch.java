		...
		*/
		...
		while (true){
			...
			{//这一部分进行判断的是每一个刚发出的请求它有哪些旁边2*2的出租车满足要求，并把满足要求的taxi放入每一个请求的set里(放入set是防止重复
				...
				first_flag = false;
				boolean u = !rq.getused(j);
				long ti = rq.gettime(j);
				if (u && time_now >= ti && time_now <= ti+3000)
				{
					boolean f = rq.getfirst(j);
					if (f){//每次只要输出一次，因此引入了布尔量first
						rq.setfirst(j);
...
gui.RequestTaxi(new Point(rq.getsrcx(j)-1,rq.getsrcy(j)-1), new Point(rq.getdstx(j)-1,rq.getdsty(j)-1));//读入大量请求，请注释掉这句话！！！			
					...
					for (int i = 1; i <= 100; ++i){//if (rq.getfirst(j))//System.out.println(i+" $$$"+" "+c[i].getx()+" "+" "+c[i].gety()+"            "+c[i].getstate());
						tmp = myabs(c[i].getx()-rq.getsrcx(j))<=2 && myabs(c[i].gety()-rq.getsrcy(j))<=2;
						if (tmp){
							sta = c[i].getstate();
							if (sta == 2){
			...
			for (int j = 0; j < totnum; ++j){
				tmp = !rq.getused(j) && time_now > rq.gettime(j) + 3000;
				if (tmp){
					...
					for(Integer k : se){
			            ...
			            if (sta == 2){
			            	if (sign == false){
			            		...
			            		tmp =  (c[k].getcredit() > c[num].getcredit()) || (c[k].getcredit() == c[num].getcredit()
			            				&& dist < dismin);
			            		if (tmp){
			            				num = k;
			            				dismin = dist;
