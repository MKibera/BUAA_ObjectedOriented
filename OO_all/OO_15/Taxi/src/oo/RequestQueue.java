...
/**
 * @startuml
 * [*] -> 初始乘客请求状态 : RequestAdd.run()
 * 初始乘客请求状态 : 一个合法的乘客请求，还没有被接单
 * 初始乘客请求状态 -> 给附近的出租车抢单 : dispatch.work()
 * 给附近的出租车抢单 : 3秒内乘客附近2*2的出租车可以抢单
 * 给附近的出租车抢单 -> 分配成功 : rq.setused(j,true)
 * 分配成功 : 分配给了符号条件的出租车，并从中选出第i号车
 * 分配失败 : 没有符合条件的出租车
 * 给附近的出租车抢单 -> 分配失败 : rq.setused(j,false)
 * 分配成功 -> 到达目的地 : car[i].run()
 * 到达目的地 : 第i号出租车处于服务状态送乘客至目的地
 * 到达目的地 -> [*]
 * 分配失败 -> [*]
 * note "乘客请求状态图" as N1
 * @enduml
